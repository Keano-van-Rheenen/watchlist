<x-layouts::app :title="__('Edit Movie')">
	@vite(['resources/css/app.css', 'resources/js/app.js'])

	<div class="mx-auto w-full max-w-2xl p-6">
		<h1 class="mb-6 text-2xl font-semibold">Update Movie</h1>
		...
		<form method="POST" action="{{ route('movies.update', $movie) }}" enctype="multipart/form-data" class="space-y-4">
			@csrf
			@method('PUT')
			...
			<div>
				...
				<textarea id="summary" name="summary" rows="4" required
					class="w-full rounded border border-zinc-300 p-2 text-zinc-900">{{ old('summary', $movie->summary) }}</textarea>
			</div>

			<div>
				<label for="duration" class="mb-1 block text-sm font-medium">Duration (HH:MM)</label>
				<input id="duration" name="duration" type="text" value="{{ old('duration', $movie->duration) }}" placeholder="01:30" required
					class="w-full rounded border border-zinc-300 p-2 text-zinc-900" />
			</div>
